...
- And now you are good to go to run the construction. The parcellation process contains many steps, but don't worry, we already have recon-all batched script to help us to run all those steps smoothly and even realizing it. The only thing you need to do is to feed the parameters and run itFull FreeSurfer parcellation involves a hundred steps. But don't forget it will take some time depending on the processor of your pc of course, but estimated time might be a day per person in average.
